...
../../utils/mysql2sqlite.sh -u roprotokol -proprotokol -B roprotokol --default-character-set=utf8 |grep -v "PRAGMA"|grep -v TRANSACTION|grep -v "CREATE DATABASE" | perl -i -p -e 's/([^;])\n/\1/g' > ~/src/dsr.lite.sql
