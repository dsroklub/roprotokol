...
WHERE
  ...
  EXISTS (SELECT 'x' FROM Trip t, TripMember tm, TripType tt WHERE t.TripTypeID=tt.id AND tm.member_id=Member.id AND tm.TripID=t.id AND (tt.Name='Inriggerkaproning' OR tt.Name='Puls og program' OR tt.Name='Coastalkaproning' or tt.Name='Teknik og tur') AND YEAR(t.OutTime)=YEAR(NOW()))
  ORDER BY navn,ud";
