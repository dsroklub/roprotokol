...
WHERE Trip.id=TripMember.TripID AND TripType.id=Trip.TripTypeID AND Member.id=TripMember.member_id AND YEAR(OutTime)=YEAR(NOW()) AND TripType.Name="Kaninholdet" Group By Member.id ORDER BY FirstName,LastName
';
$result=$rodb->query($s) or dbErr($rodb,$res,"Error in racerkaniner query: " );
$output='xlsx';
process($result,$output,"kaninholdet","_auto");
