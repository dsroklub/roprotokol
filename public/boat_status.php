...
$s="
SELECT b.boat_name, b.boat_type, IFNULL(DamageType.name,'') as damage,out_time,location
  FROM (
  SELECT Boat.id,Boat.Name AS boat_name, Boat.Location as location, Boat.boat_type, COALESCE(MAX(Damage.Degree),0) as damage,Trip.OutTime as out_time
FROM
 Boat
...
if ($stmt = $rodb->prepare($s)) {
      ...
      print("<tr><td>".$row['boat_name']."</td><td>".$row['boat_type']."</td><td class='nr'>".$row['location']."</td><td>".$row['damage']."</td><td>".$row['out_time']."</td></tr>\n");
      }
      }  else {
