...
//$s=" SELECT ow.json,COALESCE(MAX(Damage.Degree),0) FROM (SELECT 
$s=" SELECT JSON_MERGE(ow.json , JSON_OBJECT('damage',COALESCE(MAX(Damage.Degree),0))) as json FROM (SELECT
    Boat.id  boat_id,
    Trip.id trip_id,
     JSON_MERGE(
    JSON_OBJECT(
