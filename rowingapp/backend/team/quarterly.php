...
$s=
 'SELECT team.name AS team, classdate,team.description,CONCAT(FirstName," ",LastName) AS membername, Member.MemberID
...
if ($stmt = $rodb->prepare($s)) {
    ...
    $stmt->execute();
     ...
     while ($row = $result->fetch_assoc()) {
         echo $row["membername"].",".$row["MemberID"].",".$row["team"].",".$row["classdate"]."\n";
     }
}
?> 
