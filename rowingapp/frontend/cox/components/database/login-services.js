'use strict';
angular.module('coxApp.login-services', []).service('LoginService', function($http, $q,$log) {
  ...
  this.setpw = function(data) {
    $http.post('/backend/cox/setpw.php', data).then(function(r) {
  ...
  this.check_user = function() {
    var userQ=$q.defer();
    $http.get('/backend/cox/current_user.php').then(function(r) {
      cuser.user=r.data;
