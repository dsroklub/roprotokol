'use strict';
angular.module('gymApp.database.login-services', []).service('LoginService', function($http,$q,$log) {
  ...
  this.setpw = function(data) {
    $http.post('/backend/setpw.php', data).then(function(r) {
  ...
  this.check_user = function() {
    var userQ=$q.defer();
    $http.get('/backend/current_user.php').then(function(r) {
      cuser.user=r.data;
  ...
  this.logout = function() {
    $http.get('/backend/logout.php').then(function(r) {
